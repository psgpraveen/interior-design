    ...
    <header className='shadow-md font-sans tracking-wide relative z-50'>
      ...
      <div className='flex flex-wrap items-center justify-between gap-4 px-5 lg:px-10 md:px-10 py-4 bg-white min-h-[70px]'>
        ...
        <motion.div
         ...
         animate={{ opacity: 1, x: '0%', scale: animat ? 0.9 : 1 }}
         transition={{ 
           opacity: { duration: 0.3 }, 
           x: { type: 'spring', stiffness: 50, damping: 10, duration: 0.5 }, 
           scale: { type: 'spring', stiffness: 50, damping: 10, duration: 0.5 } 
         }}
          id="collapseMenu" className={`lg:flex lg:gap-x-5 ${isMenuOpen ? 'block' : 'hidden'} max-lg:fixed max-lg:bg-white max-lg:w-1/2 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:p-6 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50`}>
